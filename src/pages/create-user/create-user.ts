...
export class CreateUserPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public alertCtrl: AlertController,public EventData: EventDataProvider, public UserGlobal: UsernameGlobalProvider, public formBuilder: FormBuilder, private menuCtrl: MenuController) {
    this.CreateUserForm = formBuilder.group({
      username: ['', Validators.compose([Validators.maxLength(15),Validators.pattern('[a-zA-Z0-9_.-]*'),Validators.required, new Validator(UserGlobal, EventData).isGlobalUsernameValid])],
      fullname: ['', Validators.compose([Validators.required,Validators.maxLength(30),Validators.pattern('[a-zA-Z]*')])],
      email: ['',Validators.compose([Validators.required,Validators.pattern('[a-z0-9_.-]+\@[a-z]+\.com'), new Validator(UserGlobal, EventData).isGlobalEmailValid])],
